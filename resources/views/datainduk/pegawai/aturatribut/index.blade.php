...
@section('content')
  ...
  var urlkiri = '{!! url("datainduk/pegawai/pegawai/aturatributdanlokasi/aturatribut/pegawai") !!}';
  var urlkanan = '{!! url("datainduk/pegawai/pegawai/aturatributdanlokasi/aturatribut/pegawaiatribut") !!}';
  ...
  function getAtributNilai() {
      ...
      $('#atributnilai').html('<option value=>-- {{ trans('all.atributnilai') }} --');
      if(atribut !== '') {
          $.ajax({
              type: "GET",
              url: '{{ url('datainduk/pegawai/pegawai/aturatributdanlokasi/aturatribut/getatributnilai') }}/' + atribut,
              data: '',
              cache: false,
              success: function (data) {
                  $('#atributnilai').html(data).css('display', '');
              }
          });
      }else{
          $('#atributnilai').html('').css('display', 'none');
      }
      return false;
  }
  ...
  function setAtribut(){
      ...
      atributnilai = atributnilai === '' ? 'o' : atributnilai;
      //load data berdasarkan atribut nilai yg dipilih
      dtable2.ajax.url('{!! url("datainduk/pegawai/pegawai/aturatributdanlokasi/aturatribut/pegawaiatribut") !!}/' + atributnilai).load();
      return false;
  }

  function submitSetAtribut(){
      ...
      var atributnilai = $('#atributnilai').val();
      if(atributnilai === ""){
          alertWarning("{{ trans('all.atributnilaikosong') }}",
                  function() {
                      aktifkanTombol();
                  });
          return false;
      ...
      $.ajax({
          ...
          success: function (token) {
              var dataString = "idpegawai=" + encodeURIComponent(s)+'&atributnilai='+atributnilai+"&_token="+token;
              $.ajax({
                  type: "POST",
                  url: "{!! url("datainduk/pegawai/pegawai/aturatributdanlokasi/aturatribut/submitsetatribut") !!}",
                  data: dataString,
                  cache: false,
                  success: function (resp) {
                      ...
                      dtable2.ajax.url(urlkanan+'/'+atributnilai).load();
                      setTimeout(function(){
          ...
          function(){
              ...
              $.ajax({
                  ...
                  success: function (token) {
                      ...
                      $.ajax({
                          type: "POST",
                          url: "{!! url("datainduk/pegawai/pegawai/aturatributdanlokasi/aturatribut/hapusatribut") !!}",
                          data: dataString,
                          cache: false,
                          success: function (resp) {
                              if (resp['status'] !== "OK") {
    ...
    <div class="row">
      <div class="col-lg-12">
        <ul class="nav nav-tabs">
          <li class="active"><a href="{{ url('datainduk/pegawai/pegawai/aturatributdanlokasi/aturatribut') }}">{{ trans('all.atribut') }}</a></li>
              ...
              <tr>
                      ...
                      <table>
                          <tr>
                              <td>
                                  ...
                                  &nbsp;&nbsp;
                                  <button @if(!Session::has('aturlokasi_atribut')) style="display:none" @else @endif type="button" id="setulang" onclick="return ke('{{ url('datainduk/pegawai/pegawai/aturatributdanlokasi/setulang/aturatribut') }}')" class="btn btn-primary"><i class="fa fa-undo"></i>&nbsp;&nbsp;{{ trans('all.setulang') }}</button>
                              </td>
                          </tr>
                      </table>
                      <p></p>
                      <div id="alertatribut" @if(!Session::has('aturatribut_atribut')) style="display:none" @endif class="alert alert-success">
                          @if(Session::has('aturatribut_atribut')) {!! Session::get('aturatribut_atribut') !!} @endif
                      </div>
                  ...
                  <td width='65px' style='text-align:center !important;margin-top:275px;padding:10px;'>
                      <table>
                          <tr>
                              <td>
                                  <button type="button" onclick="return submitSetAtribut()" class="btn btn-primary"><i class="fa fa-arrow-right"></i></button>
                              </td>
                          </tr>
                          <tr>
                              <td style="padding-top:10px">
                                  <button type="button" id="buttonhapus" onclick="return submitHapusAtribut()" class="btn btn-primary"><i class="fa fa-arrow-left"></i></button>
                              </td>
                          </tr>
                      </table>
                  </td>
                  <td valign='top' width='50%'>
                      <table>
                          <tr>
                              <td style="float:left">
                                  <select id="atribut" class="form-control" onchange="return getAtributNilai()">
                              ...
                              <td style="padding-left:10px">
                                  <select id="atributnilai" style="display:none" class="form-control" name="atributnilai" onchange="return setAtribut()"></select>
                              </td>
                          </tr>
                      </table>
                      <p></p>
...
<script>
    $(function() {
        $('#tambahatribut').click(function(){
            ...
            $.ajax({
                type: "GET",
                url: "{{ url('datainduk/pegawai/pegawai/aturatributdanlokasi/aturatribut/aturatribut') }}/"+atributnilai,
                data: '',
                async: true,
                cache: false,
                contentType: false,
                ...
                success: function(resp){
                    ...
                    var atributnilaikanan = $('#atributnilai').val();
                    _token = resp['token'];
                    dtable.ajax.url(urlkiri).load();
                    dtable2.ajax.url(urlkanan+'/'+(atributnilaikanan === '' ? 'o' : atributnilaikanan)).load();
                }
            });
            return false;
        });
