    ...
    <div class="row">
      <div class="col-lg-12">
        <ul class="nav nav-tabs">
            <li class="active"><a href="{{ url('datainduk/payroll/payrollkelompok/'.$idpayrollkelompok.'/komponenmaster') }}">{{ trans('all.komponenmaster') }}</a></li>
            <li><a href="{{ url('datainduk/payroll/payrollkelompok/'.$idpayrollkelompok.'/atribut') }}">{{ trans('all.atributnilai') }}</a></li>
        </ul>
        <br>
          <table width="100%">
              <tr>
                  @if(strpos(Session::get('hakakses_perusahaan')->payrollkomponenmaster, 't') !== false || strpos(Session::get('hakakses_perusahaan')->payrollkomponenmaster, 'm') !== false)
                      <td style="float:left">
                          <a href="{!! url('datainduk/payroll/payrollkelompok/'.$idpayrollkelompok.'/komponenmaster/create') !!}" class="btn btn-primary pull-left"><i class="fa fa-plus-circle"></i>&nbsp;&nbsp;{{ trans('all.tambahdata') }}</a>
                      </td>
                    @if($totaldata > 0)
                          <td style="float:left;margin-left:10px">
                              {{--<a href="{!! url('urutan/payrollkomponenmaster') !!}" class="btn btn-primary pull-left"><i class="fa fa-sort"></i>&nbsp;&nbsp;{{ trans('all.urutan') }}</a>--}}
                              <div class="input-group" style="margin-bottom:10px">
                                  <div class="input-group-prepend">
                                      ...
                                      <ul class="dropdown-menu" style="left:-67px !important">
                                          <li><a href="#" onclick="return ke('{!! url('urutan/payrollkomponenmasterurutanperhitungan/'.$idpayrollkelompok) !!}')">{{ trans('all.digunakan') }}</a></li>
                                          <li><a href="#" onclick="return ke('{!! url('urutan/payrollkomponenmasterurutantampilan/'.$idpayrollkelompok) !!}')">{{ trans('all.tampilan') }}</a></li>
                                      </ul>
                                  </div>
                              </div>
                          </td>
                  ...
                  <td>
                      <button class="btn btn-primary pull-right" onclick="return ke('{{ url('payrollkomponenmaster/'.$idpayrollkelompok.'/excel') }}')"><i class="fa fa-download"></i>&nbsp;&nbsp;{{ trans('all.ekspor') }}</button>
                  </td>
              </tr>
          </table>
          <p></p>
...
$(function() {
    $('.datatable').DataTable({
        ...
        scrollX: true,
        ajax: {
            url: '{!! url("datainduk/payroll/payrollkelompok/".$idpayrollkelompok."/komponenmaster/index-data") !!}',
            type: "POST",
