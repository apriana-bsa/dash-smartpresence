    ...
    <div class="row">
      <div class="col-lg-12">
          <ul class="nav nav-tabs">
              <li class="active"><a href="{{ url('datainduk/absensi/indexjamkerja') }}">{{ trans('all.jamkerja') }}</a></li>
              <li><a href="{{ url('datainduk/absensi/indexlembur') }}">{{ trans('all.lembur') }}</a></li>
          </ul>
          <br>
          <table width="100%">
              <tr>
                  @if(strpos(Session::get('hakakses_perusahaan')->mesin, 't') !== false || strpos(Session::get('hakakses_perusahaan')->mesin, 'm') !== false)
                      <td style="float:left">
                          <a href="{!! url('datainduk/absensi/indexjamkerja/create') !!}" class="btn btn-primary pull-left"><i class="fa fa-plus-circle"></i>&nbsp;&nbsp;{{ trans('all.tambahdata') }}</a>
                      </td>
                  @endif
                  <td>
                      <button class="btn btn-primary pull-right" onclick="return ke('{{ url('indexjamkerja/excel') }}')"><i class="fa fa-download"></i>&nbsp;&nbsp;{{ trans('all.ekspor') }}</button>
                  </td>
              </tr>
          </table>
          <p></p>
...
$(function() {
    $('.datatable').DataTable({
        ...
        scrollX: true,
        ajax: {
            url: '{!! url("datainduk/absensi/indexjamkerja/index-data") !!}',
            type: "POST",
