...
$.fn.rangeCalendar = function(options) {
    ...
    var defaults = {
			...
			theme: "default-theme",
			themeContext: this,
			startDate: moment().add('months', -24),
	...
	this.each(function(i, el) {
		...
		obj.setStartDate = function(startDate) {
			...
			obj._placeElement(obj.monthsObj,monthCell.position());
		...
		obj._placeElement = function (el, position) {
			var calendarViewWidth = $el.outerWidth()-52;
		...
		obj._init = function( element,options ) {
			...
			obj.start = (!options.start ? "+0" : parseInt(options.start)) ;
