...
@section('title', trans('all.kehadiran'))
@section('content')
    ...
    <div class="row wrapper border-bottom white-bg page-heading">
        <div class="col-lg-10">
            <h2>{{ trans('all.kehadiran') }}</h2>
            <ol class="breadcrumb">
                <li>{{ trans('all.laporan') }}</li>
                <li class="active"><strong>{{ trans('all.kehadiran') }}</strong></li>
            </ol>
        </div>
        <div class="col-lg-2">
    ...
    <div class="wrapper wrapper-content animated fadeIn">
        <div class="row">
            <div class="col-lg-12">
                @if($data == '')
                    <form action="{{ url('laporan/kehadiran') }}" method="post" onsubmit="return validasi()">
                        {{ csrf_field() }}
                        <table>
                            <tr>
                                ...
                                <td style="float:left">
                                    <select id="filtermode" name="filtermode" class="form-control" onchange="return lapFilterMode()">
                                        @if(Session::has('lapkehadiran_filtermode'))
                                            <option value="jangkauantanggal" @if(Session::get('lapkehadiran_filtermode') == 'jangkauantanggal') selected @endif>{{ trans('all.jangkauantanggal') }}</option>
                                            <option value="berdasarkantanggal" @if(Session::get('lapkehadiran_filtermode') == 'berdasarkantanggal') selected @endif>{{ trans('all.berdasarkantanggal') }}</option>
                                        @else
                        ...
                        <table width="100%" id="jangkauantanggal" style="display: none;">
                            <tr>
                                ...
                                <td style="float:left">
                                    <input type="text" name="tanggalawal" size="11" id="tanggalawal" @if(Session::has('lapkehadiran_tanggalawal')) value="{{ Session::get('lapkehadiran_tanggalawal') }}" @else value="{{ $valuetglawalakhir->tanggalawal }}" @endif class="form-control date" placeholder="dd/mm/yyyy">
                                </td>
                                <td style="float:left;margin-top:8px">-</td>
                                <td style="float:left">
                                    <input type="text" name="tanggalakhir" size="11" id="tanggalakhir" @if(Session::has('lapkehadiran_tanggalakhir')) value="{{ Session::get('lapkehadiran_tanggalakhir') }}" @else value="{{ $valuetglawalakhir->tanggalakhir }}" @endif class="form-control date" placeholder="dd/mm/yyyy">
                                </td>
                                <td style="float:left">
                                    ...
                                    <button type="button" id="setulang" onclick="ke('setulang/kehadiran')" class="submit btn btn-primary"><i class="fa fa-undo"></i>&nbsp;&nbsp;{{ trans('all.setulang') }}</button>
                                </td>
                            </tr>
                        </table>
                        ...
                        <p><b>{{ trans('all.jenis') }}</b></p>
                        {{ $checked_terlambat = false }}
                        {{ $checked_pulangawal = false }}
                        {{ $checked_lamalembur = false }}
                        @if(Session::has('lapkehadiran_jenis'))
                            @for($i=0;$i<count(Session::get('lapkehadiran_jenis'));$i++)
                                @if(Session::get('lapkehadiran_jenis')[$i] == 'terlambat')
                                    <span style="display:none">{{ $checked_terlambat = true }}</span>
                                @endif
                                @if(Session::get('lapkehadiran_jenis')[$i] == 'pulangawal')
                                    <span style="display:none">{{ $checked_pulangawal = true }}</span>
                                @endif
                                @if(Session::get('lapkehadiran_jenis')[$i] == 'lamalembur')
                                    <span style="display:none">{{ $checked_lamalembur = true }}</span>
                                @endif
                            @endfor
                        @endif
                        <div class="col-md-2">
                            <table>
                                <tr>
                                    <td valign="top" style="width:10px;">
                                        <input type="checkbox" id="terlambat" @if($checked_terlambat == true) checked @endif name="jenis[]" value="terlambat">
                                    </td>
                                    <td valign="top">
                                        <span class="spancheckbox" onclick="spanClick('terlambat')">{{ trans('all.terlambat') }}</span>
                                    </td>
                                </tr>
                            </table>
                        </div>
                        <div class="col-md-2">
                            <table>
                                <tr>
                                    <td valign="top" style="width:10px;">
                                        <input type="checkbox" id="pulangawal" @if($checked_pulangawal == true) checked @endif name="jenis[]" value="pulangawal">
                                    </td>
                                    <td valign="top">
                                        <span class="spancheckbox" onclick="spanClick('pulangawal')">{{ trans('all.pulangawal') }}</span>
                                    </td>
                                </tr>
                            </table>
                        </div>
                        <div class="col-md-2">
                            <table>
                                <tr>
                                    <td valign="top" style="width:10px;">
                                        <input type="checkbox" id="lamalembur" @if($checked_lamalembur == true) checked @endif name="jenis[]" value="lamalembur">
                                    </td>
                                    <td valign="top">
                                        <span class="spancheckbox" onclick="spanClick('lamalembur')">{{ trans('all.lamalembur') }}</span>
                                    </td>
                                </tr>
                            </table>
                        </div>
                ...
                @else
                    <div class="ibox float-e-margins">
                        @if($totaldata > 0)
                            <button type="button" class="btn btn-primary pull-right" onclick="return ke('{{ url('laporan/kehadiran/excel') }}')"><i class="fa fa-download"></i>&nbsp;&nbsp;{{ trans('all.ekspor') }}</button><p></p>
                        @endif
                        <button onclick="ke('kehadiran')" class="btn btn-primary"><i class="fa fa-undo"></i>&nbsp;&nbsp;{{ trans('all.kembali') }}</button>
                        <p></p>
                        ...
                        <div class="ibox-content">
                            <table width=100% class="table datatable table-striped table-condensed table-hover">
                                ...
                                <tr>
                                    ...
                                    <td class="alamat"><b>{{ trans('all.alasantidakmasuk') }}</b></td>
                                    <td class="alamat"><b>{{ trans('all.jamkerja') }}</b></td>
                                    <td class="opsi3"><b>{{ trans('all.alasanmasuk') }}</b></td>
                                    <td class="alamat"><b>{{ trans('all.lokasimasuk') }}</b></td>
                                    <td class="alamat"><b>{{ trans('all.lokasikeluar') }}</b></td>
                                    <td class="opsi4"><b>{{ trans('all.waktumasuk') }}</b></td>
                                    <td class="opsi4"><b>{{ trans('all.waktukeluar') }}</b></td>
                                    <td class="opsi2"><b>{{ trans('all.datangawal') }}</b></td>
                                    ...
                                    <td class="opsi3"><b>{{ trans('all.lamaflexytime') }}</b></td>
    ...
    @if($data != '')
        $(function() {
            $('.datatable').DataTable({
                ...
                scrollX: true,
                ajax: {
                    url: '{!! url("laporan/kehadiran/index-data") !!}',
                    type: "POST",
                ...
                columns: [
                    ...
                    { data: 'lokasimasuk', name: 'lokasimasuk' },
                    { data: 'lokasikeluar', name: 'lokasikeluar' },
