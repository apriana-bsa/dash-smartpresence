@if($more == '')
    @if($totaldata > $limitdata)
        ...
        $(document).ready(function() {
            ...
            win.scroll(function() {
                ...
                //console.log(height+' '+win.scrollTop()+' '+document.getElementById("bodymodaljadwalshift").scrollHeight);
                if (document.getElementById("bodymodaljadwalshift").scrollHeight - 400 == win.scrollTop()) {
                    if(run == true) {
                        run = false;
                        loadMore();
                        return false;
            ...
            function loadMore(){
                ...
                if(startfrom != '') {
                    $('#startfrom').remove();
                    var url = '{{ url('jadwalshift/'.$tanggal.'/'.$jenis.'/'.$idjenis) }}/' + startfrom;
                    $.ajax({
                        type: "GET",
                        url: url,
                        data: '',
                        ...
                        success: function (html) {
                            $('#bodymodaljadwalshift').append(html);
    ...
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title">{{ trans('all.jadwalshift').' ('.trans('all.'.$jenis).')' }}</h4>
        </div>
            ...
            @foreach($data as $key)
                <div class="_datamore col-lg-2 col-md-3 col-sm-4 col-xs-6" style="margin-bottom:20px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">
                    <center>
                        ...
                        <br><span style="font-size:11px">{{ $key->pin }}</span>
                        @if($jenis == 'sudahabsen')
                            <br><span style="font-size:11px">{{ $key->waktumasuk }}</span>
                        @elseif($jenis == 'belumabsen')
                            {{--@if($key->atribut != '')--}}
                                {{--<br><span style="font-size:11px">{{ $key->atribut}}</span>--}}
                            {{--@else <br>&nbsp; @endif--}}
                        @endif
                    </center>
                </div>
