...
@section('content')
  ...
  <div class="wrapper wrapper-content animated fadeIn">
    <div class="row">
      @if($data != '')
        <form action="{{ url('datainduk/payroll/payrollkomponeninputmanual/submitsimpan') }}" method="post" @if($tipedata != 'teks') onsubmit="return validasi()" @endif>
          <div class="ibox float-e-margins">
            ...
            @if(strpos(Session::get('hakakses_perusahaan')->payrollkomponeninputmanual, 'u') !== false || strpos(Session::get('hakakses_perusahaan')->payrollkomponeninputmanual, 'm') !== false)
            ...
            <button type="button" id="ekspor" class="btn btn-primary pull-right" onclick="ke('{{ url('datainduk/payroll/payrollkomponeninputmanual/excel') }}')"><i class="fa fa-download"></i>&nbsp;&nbsp;{{ trans('all.ekspor') }}</button>
            <br><p></p>
  ...
  {{--  form kembali  --}}
  <form action="{{ url('datainduk/payroll/payrollkomponeninputmanual/nextstep') }}" method="post">
    {{ csrf_field() }}
    <input type="hidden" value="{{ Session::get('payrollkomponeninputmanual_bulan') }}" name="bulan">
    <input type="hidden" value="{{ Session::get('payrollkomponeninputmanual_tahun') }}" name="tahun">
    @if(Session::has('payrollkomponeninputmanual_atribut'))
      <input type="hidden" value="{{ implode(Session::get('payrollkomponeninputmanual_atribut'),"|") }}" name="atributnilai">
    @else
  ...
  <script>
    ...
    function setSemuaPegawai(jenis){
      ...
      if(pilihan == 'inputmanual'){
        ...
        var url = '{{ url("payrollgetinput")}}/'+periode;
        $.ajax({
          type: "GET",
          url: url,
          data: '',
