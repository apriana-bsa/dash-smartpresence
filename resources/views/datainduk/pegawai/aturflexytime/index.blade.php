...
@section('title', trans('all.flexytime'))
@section('content')
  ...
  var urlkiri = '{!! url("datainduk/pegawai/pegawai/aturatributdanlokasi/aturflexytime/index-data/t") !!}';
  var urlkanan = '{!! url("datainduk/pegawai/pegawai/aturatributdanlokasi/aturflexytime/index-data/y") !!}';
  ...
  function submit(flexytime){
      ...
      $.ajax({
          ...
          success: function (token) {
              var dataString = "idpegawai=" + encodeURIComponent(s)+'&flexytime='+flexytime+"&_token="+token;
              $.ajax({
                  type: "POST",
                  url: "{!! url("datainduk/pegawai/pegawai/aturatributdanlokasi/aturflexytime/submit") !!}",
                  data: dataString,
                  cache: false,
    ...
    <div class="row">
      <div class="col-lg-12">
        <ul class="nav nav-tabs">
          ...
          <li class="active"><a href="{{ url('datainduk/pegawai/pegawai/aturatributdanlokasi/aturflexytime') }}">{{ trans('all.flexytime') }}</a></li>
              ...
              <table>
                  <tr>
                      <td>
                          ...
                          &nbsp;&nbsp;
                          <button @if(!Session::has('aturflexytime_atribut')) style="display:none" @endif type="button" id="setulang" onclick="return ke('{{ url('datainduk/pegawai/pegawai/aturatributdanlokasi/setulang/aturflexytime') }}')" class="btn btn-primary"><i class="fa fa-undo"></i>&nbsp;&nbsp;{{ trans('all.setulang') }}</button>
                      </td>
                  </tr>
              </table>
              <div id="alertatribut" class="alert alert-success" style="margin-top:20px;margin-bottom:0;@if(!Session::has('aturflexytime_atribut')) display:none @endif">
                  @if(Session::has('aturflexytime_atribut')) {!! Session::get('aturflexytime_atribut') !!} @endif
              </div>
          </div>
          <table>
              <tr>
                  ...
                  <td valign='top' width=50%>
                      <div class="ibox float-e-margins">
                          ...
                          <div class="ibox-content">
                              <table width=100% class="table datatableflexytime table-striped table-condensed table-hover">
                                  <thead>
                                  <tr>
...
<script>
    $(function() {
        ...
        $('#tambahatribut').click(function(){
            ...
            $.ajax({
                type: "GET",
                url: "{{ url('datainduk/pegawai/pegawai/aturatributdanlokasi/aturflexytime/aturatribut') }}/"+atributnilai,
                data: '',
                async: true,
                cache: false,
                contentType: false,
        ...
        dtable2 = $('.datatableflexytime').DataTable({
            processing: true,
            bStateSave: true,
            serverSide: true,
            scrollX: true,
