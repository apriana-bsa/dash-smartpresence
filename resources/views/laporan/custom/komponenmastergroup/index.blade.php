    ...
    <div class="row">
      <div class="col-lg-12">
        <ul class="nav nav-tabs">
            <li><a href="{{ url('laporan/custom/kelompok') }}">{{ trans('all.kelompok') }}</a></li>
            <li class="active"><a href="{{ url('laporan/custom/komponenmastergroup') }}">{{ trans('all.komponenmastergrup') }}</a></li>
        </ul>
        <br>
          <table width="100%">
              <tr>
{{--                  @if(strpos(Session::get('hakakses_perusahaan')->payrollkomponenmaster, 't') !== false || strpos(Session::get('hakakses_perusahaan')->payrollkomponenmaster, 'm') !== false)--}}
                      <td style="float:left">
                          <a href="{!! url('laporan/custom/komponenmastergroup/create') !!}" class="btn btn-primary pull-left"><i class="fa fa-plus-circle"></i>&nbsp;&nbsp;{{ trans('all.tambahdata') }}</a>
                      </td>
{{--                  @endif--}}
                  <td>
                      <button class="btn btn-primary pull-right" onclick="return ke('{{ url('laporan/custom/komponenmastergroup/excel') }}')"><i class="fa fa-download"></i>&nbsp;&nbsp;{{ trans('all.ekspor') }}</button>
                  </td>
              </tr>
          </table>
          <p></p>
...
$(function() {
    $('.datatable').DataTable({
        ...
        scrollX: true,
        ajax: {
            url: '{!! url("laporan/custom/komponenmastergroup/index-data") !!}',
            type: "POST",
