...
class SaferEval {
	...
	var $disallowedVariables = array (
	);

	var $globalVariables = array (
	);

	var $allowedCalls = array (
	...
	var $disallowedExpressions = array ( 	// Probably unsafe to change these
		...
		if(empty($this->errors)) {
			...
			foreach ($this->tokens as $key => $token) {
				$i++;
					...
					switch ($id) {
						case('T_VARIABLE'): 
							if (count($this->disallowedVariables)>0 && in_array($token[1], $this->disallowedVariables) !== false) {
