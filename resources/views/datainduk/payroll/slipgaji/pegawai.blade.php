  ...
  function submit(jenis){
      ...
      $.ajax({
          ...
          success: function (token) {
              ...
              $.ajax({
                  type: "POST",
                  url: "{!! url("datainduk/payroll/slipgaji/$idslipgaji/pegawai") !!}",
                  data: dataString,
                  cache: false,
                  success: function (resp) {
                      ...
                      dtable.ajax.url( '{!! url("datainduk/payroll/slipgaji/$idslipgaji/pegawai/tersedia") !!}' ).load();
                      dtable2.ajax.url( '{!! url("datainduk/payroll/slipgaji/$idslipgaji/pegawai/tersimpan") !!}' ).load();
                      setTimeout(function(){
    ...
    <div class="row">
      <div class="col-lg-12">
          <ul class="nav nav-tabs">
              <li><a href="{{ url('datainduk/payroll/slipgaji/'.$idslipgaji.'/komponenmaster') }}">{{ trans('all.komponenmaster') }}</a></li>
              <li class="active"><a href="{{ url('datainduk/payroll/slipgaji/'.$idslipgaji.'/pegawai') }}">{{ trans('all.pegawai') }}</a></li>
          </ul>
          <br>
...
<script>
    $(function() {
        dtable = $('.datatabletersedia').DataTable({
            ...
            ajax: {
                url: '{!! url("datainduk/payroll/slipgaji/$idslipgaji/pegawai/tersedia") !!}',
                {{--type: "POST",--}}
        ...
        dtable2 = $('.datatabletersimpan').DataTable({
            ...
            language: lang_datatable,
            ajax: {
                url: '{!! url("datainduk/payroll/slipgaji/$idslipgaji/pegawai/tersimpan") !!}',
                {{--type: "POST",--}}
                {{--data: { _token: '{!! csrf_token() !!}' }--}}
            },
            columns: [
