...
@section('content')
  ...
  $(function(){
    ...
    if($('#jaraktoleransi').val() == 'ditentukan'){
        $('#tr_radius').css('display', '');
    }else{
        $('#tr_radius').css('display', 'none');
    }
      ...
      <div class="col-lg-12">
        <div class="ibox float-e-margins">
          <div class="ibox-content">
            <form action="../{{ $lokasi->id }}" method="post" onsubmit="return validasi()">
              ...
              <table width="480px">
                <tr>
                  ...
                  <td>
                    <input type="text" class="form-control" value="{{ $lokasi->nama }}" autofocus autocomplete="off" name="nama" id="nama" maxlength="30">
                  </td>
                </tr>
                <tr>
                  <td>{{ trans('all.lat') }}</td>
                  <td style="float:left">
                    <input type="text" class="form-control" size="20" value="{{ $lokasi->lat }}" autocomplete="off" name="lat" id="lat" maxlength="100">
                  </td>
                </tr>
                <tr>
                  <td>{{ trans('all.lon') }}</td>
                  <td style="float:left">
                    <input type="text" class="form-control" size="20" value="{{ $lokasi->lon }}" autocomplete="off" name="lon" id="lon" maxlength="100">
                  </td>
                </tr>
                <tr>
                    ...
                    <td style="float:left">
                        <select name="penentuanlokasi" id="penentuanlokasi" class="form-control" onchange="penentuanLokasi()">
                            <option value="radius" @if($lokasi->penentuanlokasi == 'radius') selected @endif>{{ trans('all.radius') }}</option>
                            <option value="poligon" @if($lokasi->penentuanlokasi == 'poligon') selected @endif>{{ trans('all.polygon') }}</option>
                        </select>
                    </td>
                </tr>
                <tr class="tr_radius">
                  ...
                  <td style="float:left">
                    <select name="jaraktoleransi" id="jaraktoleransi" class="form-control" onchange="setRadius()">
                      <option value="default" @if($lokasi->jaraktoleransi == 'default') selected @endif>{{ trans('all.default') }}</option>
                      <option value="ditentukan" @if($lokasi->jaraktoleransi == 'ditentukan') selected @endif>{{ trans('all.ditentukan') }}</option>
                    </select>
                  </td>
                </tr>
                <tr id="tr_radius">
                  ...
                  <td style="float:left">
                    <input type="text" class="form-control" size="5" onkeypress="return onlyNumber(0,event)" value="{{ $lokasi->radius }}" autocomplete="off" name="radius" id="radius" maxlength="5">
                  </td>
                  <td style="float:left;margin-top: 7px;">{{ trans('all.meter') }}</td>
                </tr>
                <tr>
                  <td colspan=2>
                    <button type="button" id="tombolpeta" class="btn btn-success" data-toggle="modal" data-target="#modalPeta"><i class="fa fa-map-marker"></i>&nbsp;&nbsp;{{ trans('all.peta') }}</button>&nbsp;&nbsp;
        ...
        <div>
          <table width='100%'>
            ...
            <tr>
              <td>
                <i>
                  {{ trans('all.lat') }} : <span id="latpopup">{{ $lokasi->lat }}</span><br>
                  {{ trans('all.lon') }} : <span id="lonpopup">{{ $lokasi->lon }}</span>
                </i>
              </td>
              <td align=right>
  ...
  function initMap() {
    var map = new google.maps.Map(document.getElementById('map'), {
      center: {lat: {{ $lokasi->lat }}, lng: {{ $lokasi->lon }}},
      zoom: 13,
      mapTypeId: 'roadmap',
    ...
    // marker
    var myLatlng = new google.maps.LatLng({{ $lokasi->lat }},{{ $lokasi->lon }});
    markers = new google.maps.Marker({
      position: myLatlng,
      map: map
    });
