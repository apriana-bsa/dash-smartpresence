(function ( $ ) {
	...
	$.fn.BootSideMenu = function( options ) {
		...
		this.addClass("container sidebarfilter");

		if(side=="left"){
			this.addClass("sidebarfilter-left");
		}else if(side=="right"){
			this.addClass("sidebarfilter-right");
		}else{
			this.addClass("sidebarfilter-left");	
		}
	...
	$(document).on('click', '.sidebarfilter .list-group-item', function(){
		$('.sidebarfilter .list-group-item').each(function(){
	...
	$(document).on('click', '.sidebarfilter .list-group-item', function(event){
...
//restituisce il lato del sidebarfilter in base alla classe che trova settata
function getSide(listaClassi){
	var side;
	for(var i = 0; i<listaClassi.length; i++){
		if(listaClassi[i]=='sidebarfilter-left'){
			side = "left";
			break;
		}else if(listaClassi[i]=='sidebarfilter-right'){
			side = "right";
			break;
		}else{
			side = null;
...
//esegue l'animazione
function doAnimation(container, containerWidth, sidebarfilterSide, sidebarfilterStatus){
	var toggler = container.children()[1];
	if(sidebarfilterStatus=="opened"){
		if(sidebarfilterSide=="left"){
			container.animate({
				left:-(containerWidth+2)
			});
			toggleArrow(toggler, "left");
		}else if(sidebarfilterSide=="right"){
			container.animate({
