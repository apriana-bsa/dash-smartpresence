      ...
      <div class="col-lg-12">
        <div class="ibox float-e-margins">
          <div class="ibox-content">
          	<form action="../{{ $harilibur->id }}" method="post" onsubmit="return validasi()">
                ...
                <table width="480px">
                            ...
                            <table>
                              <tr>
                                <td style="padding-left:0px;padding-bottom: 0px;float:left">
                                  <input type="text" class="form-control date" size="11" value='{{ date_format(date_create($harilibur->tanggalawal), "d/m/Y") }}' autocomplete="off" placeholder="dd/mm/yyyy" name="tanggalawal" id="tanggalawal" maxlength="10">
                                </td>
                                <td style="padding-bottom: 0px">-</td>
                                <td style="padding-bottom: 0px;padding-right: 0px;float:left">
                                  <input type="text" class="form-control date" size="11" value='{{ date_format(date_create($harilibur->tanggalakhir), "d/m/Y") }}' autocomplete="off" placeholder="dd/mm/yyyy" name="tanggalakhir" id="tanggalakhir" maxlength="10">
                                </td>
                              </tr>
                            </table>
                        </td>
                    ...
                    <tr>
                      ...
                      <td>
                        <textarea name="keterangan" class="form-control" id="keterangan" rows="4" style="resize:none">{{ $harilibur->keterangan }}</textarea>
                      </td>
                    </tr>
                    <tr>
                            ...
                            <table id="tabelagama">
                                @if(isset($agama))
                                    @foreach($agama as $key)
                                        @if( $key->dipilih == 1)
                                            <tr>
                                                <td style="padding:2px;padding-left:10px;">{{ $key->agama }}</td>
                                            </tr>
                                        @endif
                                    @endforeach
                                @endif
							...
							<span id="agamaarea">
								@if(isset($agama))
                                    @for($i=0;$i<count($agama);$i++)
                                        @foreach($agama as $key)
                                            @if( $key->dipilih == 1)
                                                <input type='hidden' name='agama[]' value='{{ $key->id }}'>
                                            @endif
                                        @endforeach
                                    @endfor
                                @endif
                    ...
                    @if(isset($agama))
                        <div class="col-md-4">
                            <table>
                                <tr>
                                    <td style="width:20px;padding:2px" valign="top">
                                        <input type="checkbox" class="agamapopup" id="semuaagama" onclick="checkboxallclick('semuaagama','attr')" @if($jumlahagamaterpilih == count($agama)) checked @endif>
                                    </td>
                                    <td style="padding: 2px;">
                                        <span style="margin:0" id="spansemuaagama" onclick="spanallclick('semuaagama','attr')"><strong>{{ trans('all.semua') }}</strong></span>
                                    </td>
                                </tr>
                                @foreach($agama as $key)
                                    <tr>
                                        <td style="padding:2px" valign="top">
                                            <input type="checkbox" idagama="{{ $key->id }}" onchange="return checkAllAttr('attr','semuaagama')" class="agamapopup attr" name="agama_{{ $key->id }}" id="agamapopup{{ $key->id }}" value="{{ $key->id }}"  @if($key->dipilih == 1) checked @endif>
                                        </td>
                                        <td style="padding: 2px;">
