...
@section('content')
		...
		if($('#tabIndex tr td').length == 0){
				...
				$('#tabIndex tr td').each(function(){
						...
						if(success == true && pengali != undefined){
								...
								if (cekAlertAngkaValid(pengali,0,9999,2,"{{ trans('all.indexjamkerja') }}",
										function() {
												aktifkanTombol();
												setFocus($(iditem));
										}
	...
	function addIndex(){
		...
		$('#tabIndex').append("<tr id='addr_index"+i+"'>"+
														...
														"</td>"+
														"<td style=padding-left:0px;margin-top:7px;float:left>{{ trans('all.menit') }}, {{ trans('all.indexjamkerjaadalah')}} </td>"+
														"<td style=padding-left:0px;float:left>"+
      ...
      <div class="col-lg-12">
        <div class="ibox float-e-margins">
          <div class="ibox-content">
          	<form action="../{{ $indexjamkerja->id }}" method="post" onsubmit="return validasi()">
				...
				<table width="100%">
					<tr>
						...
						<td style="float:left">
							<input type="text" size="50" class="form-control" autofocus value="{{ $indexjamkerja->nama }}" autocomplete="off" name="nama" id="nama" maxlength="100">
						</td>
					</tr>
					<tr>
						<td>{{ trans('all.jenishari') }}</td>
						<td style="float:left">
							<select class="form-control" name="jenishari">
								<option value="biasa" @if($indexjamkerja->jenishari == 'biasa') selected @endif>{{ trans('all.biasa') }}</option>
								<option value="hariminggu" @if($indexjamkerja->jenishari == 'hariminggu') selected @endif>{{ trans('all.hariminggu') }}</option>
								<option value="harilibur" @if($indexjamkerja->jenishari == 'harilibur') selected @endif>{{ trans('all.harilibur') }}</option>
							</select>
						</td>
					</tr>
					<tr>
						...
						<td style="float:left">
							<input type="text" size="11" class="form-control date" autocomplete="off" value="{{ $indexjamkerja->berlakumulai }}" name="berlakumulai" id="berlakumulai" maxlength="10" placeholder="dd/mm/yyyy">
						</td>
					</tr>
					<tr>
						<td>{{ trans('all.index') }}</td>
						<td>
							<table width="50%" id='tabIndex'>
								@if($dataindex != '')
									@for($i = 0;$i<count($dataindex);$i++)
										<tr id='addr_index{{$i}}'>
												<td style=padding-left:0px;margin-top:7px;float:left>{{ trans('all.lebihdari') }}</td>
												<td style=padding-left:0px;float:left>
														<input autocomplete='off' size='10' value="{{ $dataindex[$i]['jumlahmenit'] }}" name='jumlahmenit[]' onkeypress='return onlyNumber(2,event)' placeholder='0' class='form-control jumlahmenit' type='text' id='jumlahmenit{{$i}}' maxlength='50'>
												</td>
												<td style=padding-left:0px;margin-top:7px;float:left>{{ trans('all.menit') }}, {{ trans('all.indexjamkerjaadalah')}} </td>
												<td style=padding-left:0px;float:left>
														<input autocomplete='off' name='pengali[]' value="{{ $dataindex[$i]['pengali'] }}" size='5' onkeypress='return onlyNumber(2,event)' class='form-control pengali' placeholder='0' type='text' id='pengali{{$i}}' maxlength='50'>
												</td>
												<td style=padding-left:0px;width:20px;float:left>
														<button type='button' onclick='deleteIndex({{$i}})' title='{{ trans('all.hapus') }}' class='btn btn-danger glyphicon glyphicon-remove row-remove'></button>
												</td>
								...
								@if(isset($dataatribut))
									@for($i=0;$i<count($dataatribut);$i++)
										@foreach($dataatribut[$i]['atributnilai'] as $key)
											@if($key->dipilih == 1)
												<tr>
													<td>{{ $dataatribut[$i]['atribut']." : ".$key->nilai }}</td>
												</tr>
											@endif
										@endforeach
									@endfor
									...
									@for($i=0;$i<count($dataatribut);$i++)
										@foreach($dataatribut[$i]['atributnilai'] as $key)
											@if($key->dipilih == 1)
												...
												<input type='hidden' name='idatribut[]' value='{{ $dataatribut[$i]['idatribut'] }}'>
											@endif
										@endforeach
									@endfor
								@endif
					...
					{{-- <tr>
						<td width=110px>{{ trans('all.indexjamkerja') }}</td>
						<td style="float:left">
							<input type="text" autofocus class="form-control" value="{{ $indexjamkerja->sampaidengan+0 }}" onkeypress="return onlyNumber(0,event)" size="7" name="sampaidengan" autocomplete="off" id="sampaidengan" maxlength="6">
						</td>
					...
					<tr>
						<td>{{ trans('all.index') }}</td>
						<td style="float:left">
							<input type="text" class="form-control" value="{{ $indexjamkerja->index+0 }}" onkeypress="return onlyNumber(0,event)" size="7" name="index" autocomplete="off" id="index" maxlength="6">
						</td>
					...
					<tr>
						<td colspan="3">
							...
							<button type="button" id="kembali" onclick="return ke('../../indexjamkerja')" class="btn btn-primary"><i class="fa fa-undo"></i>&nbsp;&nbsp;{{ trans('all.kembali') }}</button>
						</td>
					</tr>
				</table>
			</form>
