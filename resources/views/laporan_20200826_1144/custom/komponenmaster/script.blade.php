...
@section('content')
    ...
    <div class="col-lg-10">
      <h2>{{ trans('all.laporankomponenmaster').' ('.$data->nama.')' }}</h2>
      <ol class="breadcrumb">
						...
						<tr class="tr_inputmanual">
							...
							<td>
								...
								<div style="padding-left:0" class="col-lg-12 col-md-12 col-sm-12">
									...
									<div class="col-lg-12" style="padding-left:0;padding-top:5px;padding-bottom:10px">
										<b title="{{ trans('all.fungsigetscriptlaporan') }}" style='cursor:pointer;margin-bottom:5px' class='label pilihan' onclick='give("get(... , ...)","inputmanual_filter")'>get(... , ...)</b>&nbsp;
										<b title="{{ trans('all.fungsigetvaluescriptlaporan') }}" style='cursor:pointer;margin-bottom:5px' class='label pilihan' onclick='give("getvalue(... , ...)","formula")'>getvalue(... , ...)</b>&nbsp;
										<b title="{{ trans('all.fungsigetatributnilaiscriptlaporan') }}" style='cursor:pointer;margin-bottom:5px' class='label pilihan' onclick='give("getatributnilai(... , ...)","formula")'>getatributnilai(... , ...)</b>&nbsp;
										<b title="{{ trans('all.fungsiinarrayscriptlaporan') }}" style='cursor:pointer;margin-bottom:5px' class='label pilihan' onclick='give("inarrayi(... , ...)","inputmanual_filter")'>in_arrayi(... , ...)</b>&nbsp;
									</div>
							...
							<td>
								<div style="padding:0" class="col-lg-6 col-md-6 col-sm-12">
									...
									<div style="padding-left:0;padding-top:10px" class="col-lg-12 col-md-12 col-sm-12">
										...
										@else
											<div style="padding:5px;padding-left:0;" class="pilihpegawai col-lg-12">
												...
												<table>
													<tr>
														<td style="padding-left:0px;padding-top:0px;padding-bottom: 0px;float:left">
															<input type="text" class="form-control date" size="11" value="{{ date('d/m/Y') }}" autocomplete="off" placeholder="dd/mm/yyyy" name="tanggal" id="tanggal" maxlength="10">
														</td>
													</tr>
												</table>
											</div>
										...
										<div class="col-lg-12" style="padding-left:0;padding-top:5px;padding-bottom:10px">
											...
											<b title="{{ trans('all.variabelglobaldatalogabsen') }}" style='cursor:pointer;margin-bottom:5px' class='label pilihan' onclick='give("$LOGABSEN","formula")'>$LOGABSEN</b>&nbsp;
											...
											<b title="{{ trans('all.variabelglobaldatalaporan') }}" style='cursor:pointer;margin-bottom:5px' class='label pilihan' onclick='give("$LAPORAN","formula")'>$LAPORAN</b>&nbsp;
										</div>
										<div style="padding-left:0" class="col-lg-12"><b>{{ trans('all.fungsibantuan') }}</b></div>
										<div class="col-lg-12" style="padding-left:0;padding-top:5px;padding-bottom:10px">
											<b title="{{ trans('all.fungsigetscriptlaporan') }}" style='cursor:pointer;margin-bottom:5px' class='label pilihan' onclick='give("get(... , ...)","formula")'>get(... , ...)</b>&nbsp;
											<b title="{{ trans('all.fungsigetvaluescriptlaporan') }}" style='cursor:pointer;margin-bottom:5px' class='label pilihan' onclick='give("getvalue(... , ...)","formula")'>getvalue(... , ...)</b>&nbsp;
											<b title="{{ trans('all.fungsigetatributnilaiscriptlaporan') }}" style='cursor:pointer;margin-bottom:5px' class='label pilihan' onclick='give("getatributnilai(... , ...)","formula")'>getatributnilai(... , ...)</b>&nbsp;
											<b title="{{ trans('all.fungsiinarrayscriptlaporan') }}" style='cursor:pointer;margin-bottom:5px' class='label pilihan' onclick='give("inarrayi(... , ...)","formula")'>in_arrayi(... , ...)</b>&nbsp;
										</div>
										<div style="padding-left:0" class="col-lg-12"><b>{{ trans('all.atributnilai') }}</b></div>
										<div class="col-lg-12" style="padding-left:0;padding-top:5px;padding-bottom:10px">
											@if($dataatribut != '')
								...
								<div class="col-lg-6 col-md-6 col-sm-12">
									...
									<div class="col-lg-12" id="formula_komponenmaster" style="padding-left:0;padding-top:5px;padding-bottom:10px">
										@if($datalaporankomponenmaster != '')
											@foreach($datalaporankomponenmaster as $key)
												<div style="padding-top:5px">
													<div class="input_komponenmaster" style="display:inline-block">
														<input type="text" size="15" autocomplete="off" placeholder="{{ trans('all.'.$key->tipedata) }}" @if($key->tipedata == 'angka') onkeypress="return onlyNumber(0,event)" @endif idlaporankomponenmaster="{{$key->id}}" tipedata={{$key->tipedata}} id="input_{{$key->id}}" class="form-control @if($key->tipedata == 'uang') money @endif">
													</div>
													<div style="display:inline-block">
														@if($key->kode != '')
															@if($key->tipedata != 'teks')
																<b title="{{ $key->nama }}" style='cursor:pointer;margin-bottom:5px' class='label pilihan' onclick='give("get($laporan,\"{{strtoupper($key->kode)}}\",0)","formula")'>{{strtoupper($key->kode)}}</b>&nbsp;&nbsp;{{ $key->nama }}
															@else
																<b title="{{ $key->nama }}" style='cursor:pointer;margin-bottom:5px' class='label pilihan' onclick='give("get($laporan,\"{{strtoupper($key->kode)}}\",\"\")","formula")'>{{strtoupper($key->kode)}}</b>&nbsp;&nbsp;{{ $key->nama }}
															@endif
														@else
															{{ $key->nama }}
														@endif
  ...
  <form method="POST" id='formscript'>
	{{ csrf_field() }}
	<input type="hidden" value="{{$jenislaporankelompok}}" id="jenislaporankelompok" name="jenislaporankelompok">
	...
	<input type="hidden" value="" id="s_tanggal" name="tanggal">
	<input type="hidden" value="{{$data->id}}" id="idkomponenmaster" name="idkomponenmaster">
	<input type="hidden" value="{{$idlaporankelompok}}" id="idlaporankelompok" name="idlaporankelompok">
  </form>
...
<script>
	...
	function tesOutput(){
		...
		if($('#jenislaporankelompok').val() === 'rekap') {
		...
		$('.input_komponenmaster').each(function(){
			...
			komponenmaster[i] = {};
			komponenmaster[i]['id'] = $(this).find("input").attr('idlaporankomponenmaster');
			if(tipedata === 'uang'){
		...
		$('#s_tanggal').val(tanggal);
		var dataString = new FormData($('#formscript')[0]);
		$.ajax({
			type: "POST",
			url: '{{ url('laporankomponenmaster/tesoutput') }}',
			data: dataString,
			async: false,
			cache: false,
			contentType: false,
