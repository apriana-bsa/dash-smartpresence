...
class HakAksesController_penempatan extends Controller
{
    ...
    public function show(Request $request)
    {
        if (strpos(Session::get('hakakses_perusahaan')->hakakses, 'l') !== false) {
            ...
            $where = '';
            $columns = array('','nama');
            $totalData = Utils::getDataCustomWhere($pdo,'hakakses', 'count(id)', ' idperusahaan = '.Session::get('conf_webperusahaan').' AND _flaghapus = "y" ');
            ...
            if(!empty($request->input('search.value'))){
                ...
                $where .= Utils::searchDatatableQuery($search,$columns);
                $totalFiltered = Utils::getDataCustomWhere($pdo,'hakakses', 'count(id)', ' idperusahaan = '.Session::get('conf_webperusahaan').' AND _flaghapus = "y" '.$where);
            }

            $sql = 'SELECT id,nama FROM hakakses WHERE idperusahaan = '.Session::get('conf_webperusahaan').' AND _flaghapus = "y" '.$where.' ORDER BY '.$orderBy.' LIMIT '.$limit. ' OFFSET '.$start;
            ...
            if(!empty($originaldata)){
                foreach($originaldata as $key){
                    $action = '';
                    if(strpos(Session::get('hakakses_perusahaan')->hakakses, 'u') !== false || strpos(Session::get('hakakses_perusahaan')->hakakses, 'm') !== false){
                        $action .= Utils::tombolManipulasi('ubah','hakakses',$key['id']);
                    }
                    if(strpos(Session::get('hakakses_perusahaan')->hakakses, 'h') !== false || strpos(Session::get('hakakses_perusahaan')->hakakses, 'm') !== false){
    ...
    {
        if(strpos(Session::get('hakakses_perusahaan')->hakakses, 't') !== false || strpos(Session::get('hakakses_perusahaan')->hakakses, 'm') !== false){
    ...
    public function store(Request $request)
    {
        ...
        $penempatan = '';
        if(isset($request->penempatan_lihat)){ $penempatan .= $request->penempatan_lihat; }
        if(isset($request->penempatan_tambah)){ $penempatan .= $request->penempatan_tambah; }
        if(isset($request->penempatan_ubah)){ $penempatan .= $request->penempatan_ubah; }
        if(isset($request->penempatan_hapus)){ $penempatan .= $request->penempatan_hapus; }
        $facesample = '';
        ...
        $penempatan .= strpos($penempatan, 'ltuh') !== false ? 'm' : '';
        ...
        if ($stmt->rowCount()==0) {
            ...
            $stmt->bindValue(':penempatan', $penempatan);
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($stmt->rowCount()>0) {
            ...
            if ($stmt->rowCount()==0) {
                ...
                $hakaksess->penempatan = $penempatan;
