...
@section('content')
      ...
      <div class="col-lg-12">
        <div class="ibox float-e-margins">
          <div class="ibox-content">
          	<form action="../{{ $batasan->id }}" method="post" onsubmit="return validasi()">
              ...
              <table width="480px">
                <tr>
                    ...
                    <td>
                        <input type="text" class="form-control" value="{{ $batasan->namabatasan }}" autofocus autocomplete="off" name="namabatasan" id="namabatasan" maxlength="100">
                    </td>
                </tr>
                <tr>
                  ...
                  <td style="float: left;">
                    <table id="tabelatribut">
                      @foreach($atributs as $atribut)
                        @if($atribut->pakaiheader > 0)
                          <tr>
                            <td style="padding:2px"><b>{{ $atribut->atribut }}</b></td>
                          </tr>
                        @endif
                        @foreach($atribut->atributnilai as $atributnilai)
                          @if( $atributnilai->dipilih == 1)
                            <tr>
                              <td style="padding:2px;padding-left:10px;">{{ $atributnilai->nilai }}</td>
                            </tr>
                          @endif
                        @endforeach
                      @endforeach
                    ...
                    <span id="atributarea">
                      @foreach($atributs as $atribut)
                        @foreach($atribut->atributnilai as $atributnilai)
                          @if( $atributnilai->dipilih == 1)
                            <input type='hidden' name='atribut[]' value='{{ $atributnilai->id }}'>
                          @endif
                        @endforeach
                      @endforeach
                    </span>
                ...
                <tr>
                  ...
                  <td style="float: left;">
                    <table id="tabelpekerjaankategori">
                      @if($datapekerjaankategori != '')
                        @foreach($datapekerjaankategori as $key)
                          @if( $key->dipilih == 1)
                            <tr>
                              <td style="padding:2px;">{{ $key->nama }}</td>
                            </tr>
                          @endif
                        @endforeach
                      @endif
                    ...
                    <span id="pekerjaankategoriarea">
                      @foreach($datapekerjaankategori as $key)
                        @if( $key->dipilih == 1)
                          <input type='hidden' name='pekerjaankategori[]' value='{{ $key->id }}'>
                        @endif
                      @endforeach
                    </span>
                  </td>
              ...
              <table>
                @if($datapekerjaankategori != '')
                  @foreach($datapekerjaankategori as $key)
                    <tr>
                      <td style="width:20px;padding:2px" valign="top">
                        <input type="checkbox" @if($key->dipilih == 1) checked @endif onchange="return checkAllAttr('pekerjaankategoripopup','semuapekerjaankategori')" class="pekerjaankategoripopup" name="pekerjaankategori_{{ $key->id }}" id="pekerjaankategoripopup{{ $key->id }}" value="{{ $key->id }}">
                      </td>
                      <td style="padding: 2px;">
  ...
  $(function(){
    setTimeout(checkAllAttr('pekerjaankategoripopup','semuapekerjaankategori'),200);
