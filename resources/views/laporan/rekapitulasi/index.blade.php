...
@section('content')
  ...
  <div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-10">
      <h2>{{ trans('all.rekapitulasi') }}</h2>
      <ol class="breadcrumb">
        <li>{{ trans('all.laporan') }}</li>
        <li class="active"><strong>{{ trans('all.rekapitulasi') }}</strong></li>
      </ol>
    </div>
    <div class="col-lg-2">
  ...
  <div class="wrapper wrapper-content animated fadeIn">
    <div class="row">
      <div class="col-lg-12">
        @if($data == '')
          <form action="{{ url('laporan/rekapitulasi') }}" method="post" onsubmit="return validasi()">
            {{ csrf_field() }}
            <table>
              <tr>
            ...
            <table width="100%" id="jangkauantanggal" style="display: none;">
              <tr>
                ...
                <td style="float:left">
                  <input type="text" name="tanggalawal" size="11" id="tanggalawal" @if(Session::has('laprekapitulasi_tanggalawal')) value="{{ Session::get('laprekapitulasi_tanggalawal') }}" @else value="{{ $valuetglawalakhir->tanggalawal }}" @endif class="form-control date" placeholder="dd/mm/yyyy">
                </td>
                <td style="float:left;margin-top:8px">-</td>
                <td style="float:left">
                  <input type="text" name="tanggalakhir" size="11" id="tanggalakhir" @if(Session::has('laprekapitulasi_tanggalakhir')) value="{{ Session::get('laprekapitulasi_tanggalakhir') }}" @else value="{{ $valuetglawalakhir->tanggalakhir }}" @endif class="form-control date" placeholder="dd/mm/yyyy">
                </td>
                <td style="float:left">
                  ...
                  <button type="button" id="setulang" onclick="ke('setulang/rekapitulasi')" class="btn btn-primary"><i class="fa fa-undo"></i>&nbsp;&nbsp;{{ trans('all.setulang') }}</button>
                </td>
              </tr>
            </table>
                  ...
                  <div class="col-md-4">
                      ...
                      @foreach($atribut->atributnilai as $atributnilai)
                        @if(Session::has('laprekapitulasi_atribut'))
                          {{ $checked = false }}
                          @for($i=0;$i<count(Session::get('laprekapitulasi_atribut'));$i++)
                            @if($atributnilai->id == Session::get('laprekapitulasi_atribut')[$i])
        ...
        @else
          <div class="ibox float-e-margins">
            @if($totaldata > 0)
                <button type="button" class="btn btn-primary pull-right" onclick="return ke('{{ url('laporan/rekapitulasi/excel') }}')"><i class="fa fa-download"></i>&nbsp;&nbsp;{{ trans('all.ekspor') }}</button><p></p>
            @endif
            <button onclick="ke('rekapitulasi')" class="btn btn-primary"><i class="fa fa-undo"></i>&nbsp;&nbsp;{{ trans('all.kembali') }}</button>
            <p></p>
...
@if($data != '')
    $(function() {
        $('.datatable').DataTable({
            ...
            serverSide: true,
            scrollX: true,
            //ajax: '{!! url("laporan/rekapitulasi/index-data") !!}',
            ajax: {
                url: '{!! url("laporan/rekapitulasi/index-data") !!}',
                type: "POST",
